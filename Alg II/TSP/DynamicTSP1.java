...
public class DynamicTSP1 {

//public static final int v_size= 8;
//public static final int num_of_subsets = (int)Math.pow(2,v_size-1);
//static float [][] A = new float [num_of_subsets][v_size+1];	
//static float[][] DistMatrix = new float[v_size+1][v_size+1]; 
//static float[][] cities = new float [v_size+1][2];	
//static int [] cardinality = new int[num_of_subsets];
...
return (intValue != 0);
 ...
 public static BitSet convert(int value) {
  ...
  public static void make_v_list(int sn, LinkedList List, int v_size){
  ...
  public static int get_subset_number(int csn, int j, int v_size){
	BitSet bits = new BitSet(v_size-1);
	bits = convert(csn);
	bits.clear(v_size-j);
...
public static void main(String args[])throws java.io.IOException
	...
	float[][] DistMatrix = new float[1][1];
	float[][] cities = new float[1][1] ;
	...
	int  v_size=0, index=0, vertex=0, distance=0;
	float x=0, y=0, z=0, w=0;
	try{
		Scanner scanner = new Scanner(new File(filename));
		v_size = scanner.nextInt(); // number of vertices
		...
		index++;
		while(scanner.hasNextFloat() && index < v_size+1 ){
	...
	final int num_of_subsets = (int)Math.pow(2,v_size-1);
	float [][] A = new float [num_of_subsets][v_size+1];	
	...
	for (int m=2; m<=v_size; m++) {
		...
		for (int sn=0; sn<index2; sn++){
			...
			v_list.clear();
			current_subset_number=Subsets_of_size[m].removeFirst(); //get(sn);
			make_v_list( current_subset_number, v_list, v_size);
			index1= v_list.size();
				...
				for (int vi=0; vi<index1; vi++){
					...
					min=inf;
						for (int ki=0; ki<index1; ki++){
							...
							if ((current_k != current_j) && (current_j !=1)){
								calc = A[get_subset_number(current_subset_number, current_j, v_size)][current_k] + 
					...
					if(args.length > 2) {
					System.out.println("writing A[" + current_subset_number +"]["+current_j + "] = " + min);}
	...
	for (int i=2; i<=v_size;i++){
		...
		if(args.length > 1) System.out.println("i= " +i+ " A[S][" +i +"] = " + A[num_of_subsets-1][i] + 
				" + " + DistMatrix[i][1] + " = " + final_tsp);
		if (final_tsp < final_min) final_min=final_tsp;
		
	}
	System.out.println("Final TSP tour = " + final_min);
